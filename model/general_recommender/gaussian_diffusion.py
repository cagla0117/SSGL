...
class GaussianDiffusion(nn.Module):
    def __init__(self, mean_type, noise_schedule, noise_scale, noise_min, noise_max,\
        ...
        if isinstance(steps, tuple):
            steps = steps[0]
        if isinstance(history_num_per_term, tuple):
            history_num_per_term = history_num_per_term[0]
        ...
        # eski (hatalı)
# self.Lt_history = th.zeros((steps, history_num_per_term), dtype=th.float64, device=device)

# yeni (uyumlu ve çalışır)
        

        self.Lt_history = th.zeros((steps, history_num_per_term), device=device, dtype=th.float64)
        self.Lt_count = th.zeros((steps,), device=device, dtype=th.int64)
